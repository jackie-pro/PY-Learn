...
while line8 != '':
    ...
    print(year3)
    mon1 = eval(data8[1]) #取出調價日期的月份
    print(mon1)
    day3 = eval(data8[2])  #取出調價日期的日
    print(day3)
    if mon1 < 10:
        nmon1 = '%02d' % mon1
        out_temp_open_score_file1.write('%s/%s/' % (year3,nmon1))
        print('%s/%s/' % (year3,nmon1))
    else:
        out_temp_open_score_file1.write('%s/%s/' % (year3,mon1))
        print('%s/%s/' % (year3,mon1))
        
    if day3 < 10:
        nday3 = '%02d' % day3
        out_temp_open_score_file1.write('%s' % (nday3))
        print('%s' % (nday3))
    else:
        out_temp_open_score_file1.write('%s' % day3)
        print('%s' % day3)
    out_temp_open_score_file1.write(',%s' % data9)
    line8 = in_open_score_file2.readline()
in_open_score_file2.close()
out_temp_open_score_file1.close()

#==日期排序 由下往上反轉=====================
in_open_score_file3 = pd.read_csv(r'C:\GitHub\python\PY-Learn\oil-project\open-score-temp.csv' ,encoding='utf8')
print(in_open_score_file3)
csvtmp = [in_open_score_file3]
print(csvtmp)
sorted_csv = sorted(csvtmp)
print(sorted_csv)
sorted_csv.to_csv(r'C:\GitHub\python\PY-Learn\oil-project\open-score-temp1.csv', encoding='utf8')
#===========================
'''
